...
% This is OULTON 2010 WITH GROWTH AND WITH NEWS SHOCKS

function [fyn, fxn, fypn, fxpn] = model_news(param)

%Steady State
[ss, param] = model_news_ss(param);
...
syms V1 V1_p V2 V2_p % news shocks
...
% KC KI GAMC GAMI
X  = [KC KI GAMC GAMI V1 V2]; % vector of state variables  
XP = [KC_p KI_p GAMC_p GAMI_p V1_p V2_p]; % p signifies t+1 
...
f(end+1)= -1 + bet*C/C_p*(1+GP_p)/(1+G)*(RI_p/(1+GP_p) + 1-di); % yellow 1
...
f(end+1)= log(GAMC_p/gamc) - .95*log(GAMC/gamc) - V1; %taken directly from Ryan's example code.
