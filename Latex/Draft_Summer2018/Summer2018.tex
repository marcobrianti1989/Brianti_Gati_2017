...
with 
\begin{eqnarray*}
U(c_t,l_t) = \log(c_t) - \frac{1}{1+\eta} l_t^{1+\eta}
\end{eqnarray*}
where $c_t$ and $l_t$ represent consumption and labor at time $t$. 

\subsection{Final-Good Sector}

In each period, the production of final output $y^c_t$ requires the services of labor $l_{t}$, and two types of capital: hard capital, $k^c_{t}$ and ICT capital, $k^i_{t}$. In addition, production function is augmented with (i) neutral productivity common across sectors, (ii) sector-specific productivity, and (iii) a positive spillover effect related to the diffusion of ICT capital in the whole economy. Production of final output is undertaken in accordance with
\begin{eqnarray}\label{equation:production_FINAL}
y^c_t = A^c_t \ (k^c_{1,t})^a \ (k^i_{1,t})^b \ (l_{1,t})^{1-a-b}, \ \ 0 < a,b < 1
\end{eqnarray}
where total factor productivity is equal to $ A_t^c = \eta_t \ \theta^c_t \ (k^i_{t})^{\gamma} $. $\eta_t$ is the neutral technological component, $\theta^c_t$ is the sector-specific technological component, and $\gamma$ represents the parameter that governs the size of the spillover effect of ICT capital in the production function. Moreover, $k^c_{1,t}$ is the part of hard capital $k^c_t$ devoted to the final-good sector, i.e. sector 1. Similarly, $k^i_{1,t}$ and $l_{1,t}$ are the parts of ICT capital $k^i_t$ and labor $l_t$ devoted to the final-good sector, respectively. 

Final output $y^c_t$ can be used for two purposes: consumption, $c_t$ and investment in hard capital, $i^c_t$,
...
Similar to final-output production function, the creation of ICT output $y^i_t$ requires the services of labor $l_{t}$, and two types of capital: hard capital, $k^c_{t}$ and ICT capital, $k^i_{t}$. In addition, production function is augmented with (i) neutral productivity common across sectors, (ii) sector-specific productivity, and (iii) a positive spillover effect related to the diffusion of ICT capital in the whole economy. Production of ICT output is undertaken in accordance with
\begin{eqnarray}\label{equation:productionICT}
y^i_t = A^i_t \ (k^c_{2,t})^a \ (k^i_{2,t})^b \ (l_{2,t})^{1-a-b} , \ \ 0 < a,b < 1
\end{eqnarray}
where total factor productivity is equal to $ A_t^i = \eta_t \ \theta^i_t \ (k^i_{t})^{\gamma} $. $\eta_t$ is the same neutral technological component, $\theta^i_t$ is the ICT-specific technological component, and $\gamma$ represents the same parameter that governs the size of the spillover effect of ICT capital also in this production function. Similarly, $k^c_{2,t}$ is the part of hard capital $k^c_t$ devoted to the ICT-good sector, i.e. sector 2 and $k^i_{2,t}$ and $l_{2,t}$ are the parts of ICT capital $k^c_t$ and labor $l_t$ devoted to the ICT-good sector, respectively. 

ICT output $y^i_t$ can be only investment in ICT capital, $i^i_t$,
...
k^i_t = k^i_{1,t} + k^i_{2,t}, \ \ \text{and}
\end{eqnarray}
...
Competitive equilibrium is represented by (i) household's first order conditions with respect to consumption $c_t$, labor $l_t$, hard capital $k^c_t$, ICT capital $k^i_t$, (ii) final-good producers' first order conditions with respect to $k^c_{1,t}$, $k^i_{1,t}$, and $l_{1,t}$, (iii) ICT-good producers' first order conditions with respect to $k^c_{2,t}$, $k^i_{2,t}$, and $l_{2,t}$, (iv) definition of production functions \ref{equation:production_FINAL} and \ref{equation:productionICT}, (v) low of motion of hard and ICT capital \ref{equation:LOM_Hard} and \ref{equation:LOM_ICT}, (vi) sector-specific resource constraints \ref{equation:marketclearingHard} and \ref{equation:marketclearingICT}, and intrasectoral resource constraints \ref{equation:marketclearingHard}, \ref{equation:marketclearingICT}, and \ref{equation:marketclearingLabor}.
