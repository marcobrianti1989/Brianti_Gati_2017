...
Consider an economy inhabitated by a representative agent whi maximizes the expect values of his lifetime utility as given by
$$
...
In each period, the production of final output $y^c_t$ requires the services of labor $l_{1,t}$, and two types of capital: hard capital, $k^c_{1,t}$ and ICT capital, $k^i_{1,t}$.\footnote{Notice that $x_{1,t}$ has to be interpret as the part of overall input $x_t$ which is allocated to the final-output sector, i.e. sector $1$.} In addition, production function is augmented with (i) sector-specific productivity, $\Gamma^c_t$ and (ii) a positive spillover effect related to the diffusion of ICT capital in the whole economy, $k^i_t$.\footnote{Notice that differently to $k^i_{1,t}$, $k^i_t$ represents the total amount of ICT capital in the whole economy at time $t$.} Production of final output is undertaken in accordance with
$$
y^c_t = A^c_t \ (k^c_{1,t})^a \ (k^i_{1,t})^b \ (l_{1,t})^{1-a-b}
$$
$\Gamma^c_t k^i_{t} $
