...
f(end+1)= -1 + bet*C/C_p* ((1/(1+GAMC))^((1-b)/(1-a-b)) *(1/(1+GAMI))^((b)/(1-a-b)))  *(RI_p/P_p + 1-di); 
...
f(end+1)= -W + (1-a-b)*(1+GAMI)*H2^(-a-b)*KC2^(a)*KI2^(b);
f(end+1)= -RC + a*(1+GAMI)*H2^(1-a-b)*KC2^(a-1)*KI2^(b);
f(end+1)= -RI + b*(1+GAMI)*H2^(1-a-b)*KC2^(a)*KI2^(b-1);
