...
$$
\Pi_{i,j}(t) = e_i'B^t \tilde{A}_0 D e_j
$$
and its variance decomposition up to time $h$
$$
\Omega_{i,j}(h) = \frac{ \sum_{t=0}^h e_i' B^t \tilde{A}_0 D e_j e_j' D' \tilde{A}_0' B'^t e_i } {e_i'( \sum_{\tau = 0}^H B^t \Sigma B'^t )e_i}
$$
\

\textbf{Step 1 - Identification of $\gamma_{news}$}
$$
\max_{\gamma_{news}} \frac{\sum_{\tau = 0}^H B \tilde{A}_0 \tilde{A}_0 }{\sum_{\tau = 0}^H }
$$
