...
H_max = 40;
[impact, gam] = two_steps_controllingNEWS(which_variable,which_shocks,H_max,...
      LR_hor,B,A,pos_rel_prices,pos_IT);
...
for i_simul=1:nsimul
      ...
      [impact_boot(:,:,i_simul)] = two_steps_controllingNEWS(which_variable,...
            which_shocks,H_max,LR_hor,beta_tilde_star(:,:,i_simul),A_boot, pos_rel_prices,pos_IT);
end
...
N = null(gam');
D_null = [N gam];
if sum(sum((D_null*D_null' - eye(nvar)).^2)) > 10^(-10); % this is just a check, should be the identity.
      error('D matrix is not orthogonal')
end
...
[s_shock_just_IT, ~] = ...
      get_structural_shocks_general(A,gam,res,which_shocks);
