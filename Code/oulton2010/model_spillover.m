...
% Model Equations (my notes p. 274):
f(1)    = -YC + KI1^gam * (1+GAMC)*H1^(1-a-b)*KC1^(a)*KI1^(b);
...
f(end+1)= -W + KI1^gam * (1-a-b)*(1+GAMC)*H1^(-a-b)*KC1^(a)*KI1^(b); 
f(end+1)= -RC + KI1^gam * a*(1+GAMC)*H1^(1-a-b)*KC1^(a-1)*KI1^(b);
f(end+1)= -RI + KI1^gam * b*(1+GAMC)*H1^(1-a-b)*KC1^(a)*KI1^(b-1);
...
f(end+1)= -G + ((1-b -gam)*GAMC + (gam +b)*GAMI)/(1-a-b -gam);  % eq. Ialt
f(end+1)= -GI + ((a)*GAMC + (1-a)*GAMI)/(1-a-b -gam);  % eq. IIIalt
